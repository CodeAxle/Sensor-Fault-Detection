...
class DataTransformation:
    ...
    def initiate_data_transformation(self)->DataTransformationArtifact:
        try:
            ...
            target_feature_train_df.fillna(value=0, inplace=True)
            target_feature_train_df = target_feature_train_df.replace(TargetValueMapping().to_dict())
            
            #Testing Dataframe
            input_feature_test_df = test_df.drop(columns=[TARGET_COLUMN],axis=1)
            target_feature_test_df = test_df[TARGET_COLUMN]
            target_feature_test_df.fillna(value=0, inplace=True)
            target_feature_test_df = target_feature_test_df.replace(TargetValueMapping().to_dict())

            # Convert target columns to integer data type
            target_feature_train_df = target_feature_train_df.astype(int)
            target_feature_test_df = target_feature_test_df.astype(int)
            logging.info(f"train df target value data type:{target_feature_train_df.dtype}")
            logging.info(f"test df target value data type:{target_feature_test_df.dtype}")
            ...
            logging.info(f"{target_feature_train_df.unique()}")
            logging.info(f"{target_feature_train_df.nunique()}")
